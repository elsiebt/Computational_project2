...
```{r A1, fig.cap="The cummulative number of coal mining disaster per time.",fig.width=6, fig.height=4}
...
The second block keeps $t_1$ constant. The proposed new values $(\tilde\beta, \tilde\lambda_0, \tilde\lambda_1)$ are found by first generating $\tilde\beta$ from a normal distribution centered around the old value $\beta$ and using variance $\sigma_2^2$, before finding new values $(\tilde\lambda_0,\tilde\lambda_1)$ using $\tilde\beta$. The acceptance probability for the proposed values is 
...
block1 <- function(t1, lambda0, lambda1, beta, sigma.t1){
      ...
      #Metropolis-Hastings for sampling from t1
      #Generating proposal value for t1 from normal distribution
      t1.prop <-rnorm(1, t1, sigma.t1) 
...
block2 <- function(t1, lambda0, lambda1, beta, sigma.beta){
      ...
      #Metropolis-Hastings for sampling from t1
      #Generating proposal value for t1 from normal distribution
      beta.prop <-rtnorm(1, beta, sigma.beta, lower=0) 
...
From the trace plots in Figure \ref{traceblocksigma331} we observe high initial values for $\lambda_0$ and $\lambda_1$, this is problem might be caused by the burn-in period, and we are not able to see if it has converged for values around 3 and 1 because the y-axis is scaled to high. Trying to solve the problem by excluding the first 50 iterations below.
...
In Figure \ref{blockautosigma33} we observe a higher mixing properties for all parameters. For $t_1$ and $\beta$ this can be explained by the way our algorithm alternates between the two blocks. $t_1$ and $\beta$ are kept constant for every other step, so this will lead to a high correlation. For  $\lambda_0$ and $\lambda_1$ we observe higher correlation than for the single site algorithm, this might indicate that our acceptance probability is lower so that we update the values more rarely. 
...
From Figure \ref{density} we see that $\lambda_0$, $\lambda_1$ and $\beta$ follow their theoretical distribution. The density of $t_1$ is not easy to relate to a known distribution so it is hard to say if it follows a theoretical curve. However the values of $t_1$ look reasonable as most of them are in the middle of the interval which is expected since we are sampling from a normal distribution.
...
``` {r, fig.cap="\\label{dislay} Displaying the data set.",fig.width=6, fig.height=4}
...
```{r, fig.cap="\\label{hist} Histogram of the simulated $\\theta$ samples.",fig.width=6, fig.height=4}
...
gibbs_block <- function(eta, n, iter){
  ...
  alpha <- n/2
    ...
    for(i in 2:iter){
    beta <- 1+1/2*t(eta_mat[,i-1])%*%A%*%eta_mat[,i-1]
...
truehist(samples.theta, xlab=expression(theta))
```

We want to use our samples to get an estimate of the posterior marginal for the hyperparameter $\pi(\theta|y)$, so a histogram of the generated samples is displayed in Figure \ref{hist}.

Further we will use the samples to get an estimate of the smooth effect using the mean and pointwise 95% confidence interval.


```{r, fig.cap="\\label{confint} 95% confidence interval togehter with the true observations.",fig.width=6, fig.height=4}
mean_values <- rep(0,n)
var_values <- rep(0, n)
upper <-rep(0,n)
lower <-rep(0,n)
...
In Figure \ref{confint} the mean of the samples are plotted together with the confidence interval. We see that the means follow the true observations.

## B3

We want to approximate the posterior marginal for the hyperparameter $\theta$, which can be written as

$$
\begin{aligned}
\pi(\theta|\bm{y}) &\propto \frac{\pi(\bm{y}|\bm{\eta},\theta)\pi(\bm{\eta}|\theta)\pi(\theta)}{\pi(\bm{\eta}|\theta,\bm{y})} \\
& \propto \frac{ \exp\{-\frac{1}{2}(\bm{y}-\bm{\eta})^T(\bm{y}-\bm{\eta})\} \cdot \theta^{(T-2)/2}\exp\{-\frac{1}{2}\bm{\eta}^TQ(\theta)\bm{\eta}\} \cdot \exp\{-\theta\} } { \det((Q(\theta)+I)^{-1})^{-1/2}\exp\{-\frac{1}{2}(\bm{\eta}-(Q(\theta)+I)^{-1}\bm{y})^T(Q(\theta)+I)(\bm{\eta}-(Q(\theta)+I)\} } \\
&=\theta^{(T-2)/2}\det(Q(\theta)+I)^{-1/2}\exp\{-\theta-\frac{1}{2}\bm{y}^T(I-(Q(\theta)+I)^{-1})\bm{y}\}.
\end{aligned}
$$

In order to find an approximation, we use a grid of values for $\theta$ between $0$ and $6$, and calculates $\pi(\theta|\bm{y})$ for the values in the grid.

```{r, fig.cap="\\label{B3} Approxiamtion of $\\pi(\\theta|y)$ calculated with MCMC and numerical approximation.",fig.width=6, fig.height=4}
...
ggplot() + 
  geom_density(aes(x=samples.theta, colour = "MCMC samples"), fill="#FFCCCC")+
  geom_line( aes(x=theta, y=y, colour = "Num approx"))+
  xlab(expression(theta))

```

Figure \ref{B3} compares our results from the MCMC algorithm, the red line, together with our approximation on the grid, the blue line. We see that our densities match quite well.

# B4 
Next we want to look at an approximation of the marginal posterior for the smooth effect, which is
$$
\begin{aligned}
\pi(\eta_i|\bm{y})&=\int\pi(\eta_i|\bm{y},\theta)\pi(\theta|\bm{y})d\theta \\
&\approx \sum_{k=1}^N\pi(\eta_i|\theta_k,\bm{y})\pi(\theta_k|\bm{y})\Delta_\theta
\end{aligned}
$$
where $\Delta_\theta=\theta_k-\theta_{k-1}$ is the constant step size between the values of $\theta$ in the grid.

We know that $\pi(\bm{\eta}|\theta,\bm{y})\sim\mathcal{N}((Q(\theta)+I)^{-1}\bm{y}, (Q(\theta)+I)^{-1})$, and therefore we assume 
$$
\begin{aligned}
\pi(\eta_i|\theta,\bm{y})\sim\mathcal{N}\big([(Q(\theta)+I)^{-1}\bm{y}]_i, [(Q(\theta)+I)^{-1}]_{ii}\big).
\end{aligned}
$$
To calculate the approximation we use an additional grid, this one for $\eta$ and has values between $-2$ and $2$. 

```{r, fig.cap="\\label{marginal} Comparing the marginal posterior of the smooth effect of $\\pi(\\eta_{10}|y)$ calcualted from the samples and from the integral.",fig.width=6, fig.height=4}
...
ggplot() + 
  geom_density(aes(x=samples.eta[10,], colour = "Eta10 samples"), fill="#FFCCCC") +
  geom_line( aes(x=eta, y=pi, colour = "Integral approx"))+
  xlab(expression(eta))
```

Figure \ref{marginal} show our approximation of the marginal posterior for the smooth effect calculated from the samples generated via Gibbs sampling together with the numerical approximation of the integral. We see that the approximations match very well.
...
Figure \ref{inla} shows the result of using the INLA package on our model. The mean of the linear predictor overlap with the mean of $\bm\eta$, and the data points fits mostly withing the $95\%$ confidence interval. If we compare figure \ref{inla} with the figure we found in B2, figure \ref{confint}, we can see that they are very similar.  