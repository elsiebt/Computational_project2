...
```{r}
dat <- data.frame(y=df$V1, t=seq(1, length(df$V1), by=1))
formula = y ~ f(t, model = "rw2", hyper = list(prec=list(prior="loggamma", param=c(1,1))),cyclic=F,constr=F)-1
result = INLA::inla(formula = formula,data = dat,family = "gaussian",control.family = list(hyper=list(prec=list(initial=0, fixed=TRUE))),control.predictor = list(compute = TRUE, link = 1))               
```
