...
```{r, fig.cap="\\label{hist} Histogram of the simulated data. To the left we see the histogram of the $\\theta$ samples and to the right we see the  $\\eta$ samples.",fig.width=6, fig.height=4}
...
truehist(samples.eta[10,], xlab=expression(eta))
```

```{r, fig.cap="\\label{confint} 95% confidence interval.",fig.width=6, fig.height=4}
mean_values <- rep(0,n)
var_values <- rep(0, n)
upper <-rep(0,n)
lower <-rep(0,n)
...
```{r, fig.cap="\\label{integral}....",fig.width=6, fig.height=4}
...
## B5
```{r}
#Loding the INLA package
library(INLA)
```

We now want to compare our calculations to the results from the built in INLA library. 
```{r}
...
df.inla = data.frame(t = data$t, eta.mean = result$summary.random$t$mean, linear.pred.mean = result$summary.linear.predictor$mean, lower = result$summary.random$t$`0.025quant`, upper = result$summary.random$t$`0.975quant`)

ggplot(df.inla) + 
  geom_line(aes(x = t, y = eta.mean)) + 
  geom_line(aes(x = t, y = lower)) +
  geom_line(aes(x = t, y = upper)) + 
  geom_point(aes(x = t, y = df$V1))
